...
cor_data_score<-calculate_seq_correlation(geneset,pbmc_B.sub,genes_selected[1:1000],"COVID","Healthy", returnType = "score", scoreMethod = "GSCNCA")
#save(cor_data_score, file = "cor_data_score.RData")

####Calculate the network, or score of networks for permutations
cor_data_perm_list<-runPermute_seq_correlation(geneset,pbmc_B.sub,genes_selected[1:1000],"COVID","Healthy", ncores = 1, nPermute=5, returnType = "score", scoreMethod = "GSCNA")
...
cor_data_perm_list_para<-runPermute_seq_correlation(geneset,pbmc_B.sub,genes_selected,"COVID","Healthy", ncores = 4, nPermute=6, returnType = "score")

getPvalues(cor_data_score, cor_data_perm_list)
getPvalues(cor_data_score, cor_data_perm_list_para)
